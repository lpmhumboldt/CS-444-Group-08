...
// For each column of the a  matrix 
 for (int j=0; j < cols; j++){

  // extract j_th column of a |a_j> 
      matrixExtractCol(a,rows,cols,j,aColumn);

 // initializing the numerator vector
    for(int row=0;row<rows;row++){
        numerator[row] = aColumn[row];
    }

  //subtract vector projections from the extracted column of matrix a
   for(int k=0; k<j; k++){

      //extract kth Column of Q |q_k>;
      matrixExtractCol(q,rows,cols,k,qColumn);

      //compute the vector projection <q_k|a_j>|q_k>
      dotProduct = vectorDotProduct(qColumn,rows,aColumn,rows);
      vectorScale(qColumn,rows,dotProduct,projection);

      // subtract the voctor projectio
      vectorSubtract(numerator,rows,projection,rows,numerator);
   }

   //compute the norm 
     norm = vectorNorm(numerator,rows);

     if(norm>0){
    
      //normalize 
      vectorScale(numerator,rows,1/norm,numerator);

     // update jth colum of the q matrix 
         for(int row=0;row<rows;row++){
            q[j+row*cols] = numerator[row];
         }

     }else{
       printf("Error norm zero \n");
       exit(0);
    }

 } // end Q loop

 //set the R matrix to zero
 for (int row=0;row<rows;row++){
   for (int col=0;col<cols;col++){
     r[row*cols + col]=0.0;
   }
...
//Compute the R matrix
 ...
 // for each column of the r matirx
  for(int col=0;col<cols;col++){

//extract the col_th column of matrix a
   matrixExtractCol(a,rows,cols,col,aColumn);

// compute col elements in each column of r matirx
     for (int row=0;row<=col;row++){
		  matrixExtractCol(q,rows,cols,row,qColumn);
		  r[row*cols+col]= vectorDotProduct(qColumn,rows,aColumn,rows);
	 }
